...
;; ai-gftd-gameka 用に com-junkawasaki/root superproject の manifest/edn-datomize.bb
;; から移植（Phase 3 fanout）。schema-path をこの repo のルート schema.edn に
;; 向けている以外はロジック同一。
;;
  ...
  (let [[mode a b] args]
    (case mode
      ...
      "adr-file" (let [report (atom {:ok [] :skipped [] :errors [] :attrs []})]
                   ...
                   (merge-schema! (:attrs @report))
                   (println @report))
      (do (println "usage: bb scripts/edn-datomize.bb [wrap-map <path> <ns> | adr-dir <dir> | adr-file <path>]")
          (System/exit 1)))))
