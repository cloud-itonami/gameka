...
(defn- wants-json? [req]
  (let [accept (str (get-in req [:headers "accept"]))
        ct (str (get-in req [:headers "content-type"]))]
    (and (not (str/includes? accept "edn"))
         (or (str/includes? accept "json") (str/includes? ct "json")))))
...
(defn- api-key-ok? [req]
  (let [required (System/getenv "LG_API_KEY")]
    (or (str/blank? required)
        (= required (get-in req [:headers "x-api-key"])))))
...
(defn- health-handler [req]
  (data-resp req {:ok true :status "ok" :impl "clj"
                  :graphs ["health" "generate" "propose_spec" "generate_game" "playtest_game" "publish_game"]
                  :nsids (reg/nsids @registry)}))

(defn- runs-handler [req]
  (if-not (api-key-ok? req)
    (data-resp req 401 {:error "invalid api key"})
    (with-body req
      (fn [{:keys [assistant_id nsid input inputs thread_id]}]
        (let [{:keys [ok result status error]} (invoke! (or nsid assistant_id) (or input inputs {}) thread_id)]
          (if ok (data-resp req result) (data-resp req (or status 404) {:error error})))))))
...
(defn start!
  ...
  ([port]
   ...
   (println (str "gameka.gftd.ai (clj) listening on :" port))
   @server))
